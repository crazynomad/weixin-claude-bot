...
│   │   ┌──────────┐       │     │   ┌──────────────────────┐   │  │
│   │   │ HTTP 请求 │       │     │   │ 启动子进程            │   │  │
│   │   │ → API 服务│       │     │   │ → 完整的 Claude Code  │   │  │
│   │   └──────────┘       │     │   │   CLI 实例            │   │  │
│   │         │            │     │   │                      │   │  │
│   │         ▼            │     │   │ 内置 20+ 工具:       │   │  │
│   │   一段文本回复        │     │   │ Read, Edit, Write,   │   │  │
│   │                      │     │   │ Bash, Grep, Glob,    │   │  │
│   │                      │     │   │ Agent, WebSearch...   │   │  │
│   │                      │     │   └──────────────────────┘   │  │
│   └─────────────────────┘     └──────────────────────────────┘  │
│                                                                 │
│   纯对话                       带 "手" 的 AI                     │
│   Input → Text → Output       Input → 思考 → 行动 → 观察 → 循环 │
└─────────────────────────────────────────────────────────────────┘
```
...
`★ Insight ─────────────────────────────────────`
weixin-claude-bot 选择了形态 1（字符串），这意味着每条微信消息都是独立的 Claude Code 会话。好处是简单可靠，坏处是没有上下文记忆——Claude 不知道你上一条消息问了什么。如果要做多轮对话，需要用 `resume` 选项恢复之前的 session，或者切换到 AsyncIterable 模式。
`─────────────────────────────────────────────────`
...
query() 产出的消息流：
  ...
  ┌─ system (init)          ← 初始化信息：工具列表、模型、权限
  │
  ├─ user                   ← 你的输入（echo back）
  │
  ├─ assistant              ← Claude 的回复（可能包含 tool_use）
  │    ├─ text block        ← 文本思考/回复
  │    └─ tool_use block    ← 工具调用（Read, Edit, Bash...）
  │
  ├─ user (synthetic)       ← 工具执行结果（系统自动注入）
  │
  ├─ assistant              ← Claude 根据工具结果继续思考
  │    :                       （这个循环会重复 maxTurns 次）
  │
  ├─ stream_event           ← 流式 token（需开启 includePartialMessages）
  │
  ├─ system (compact)       ← 上下文压缩边界标记
  │
  └─ result                 ← 最终结果
       ├─ success           ← 成功：含 result 文本、cost、usage
...
`★ Insight ─────────────────────────────────────`
`tool()` + `createSdkMcpServer()` 的组合让你可以给 Claude Code **注入任意能力**——查数据库、调内部 API、操控硬件——而不需要单独起一个 MCP 服务器进程。这是 SDK 相比 CLI 最大的扩展性优势。weixin-claude-bot 目前没有用这两个 API，但它们是做垂直应用的关键武器。
`─────────────────────────────────────────────────`
...
```
安全性 ──────────────────────────────────────→ 自由度
 plan    default    dontAsk    acceptEdits    auto    bypassPermissions
 只读     全确认     白名单      编辑免确认    AI判断    完全放开
```
...
`★ Insight ─────────────────────────────────────`
weixin-claude-bot 选择了 `bypassPermissions` 作为默认模式，同时在文档中推荐 `auto`。这个取舍很实际：`bypassPermissions` 对所有账户类型都可用，而 `auto` 需要 Team plan。对于个人部署在自己机器上的 Bot，`bypassPermissions` + 限定 `cwd` 是一个务实的安全策略。但如果要对外提供服务，`canUseTool` 回调是最灵活的安全屏障。
`─────────────────────────────────────────────────`
...
`★ Insight ─────────────────────────────────────`
方式 4（SDK 进程内 MCP）是最轻量的集成方式——不需要额外进程，工具直接在你的 Node.js 进程中执行。如果你想给微信 Bot 加一个 "查天气" 或 "查数据库" 的能力，用 `tool()` + `createSdkMcpServer()` 几行代码就搞定，不需要单独部署 MCP 服务器。
`─────────────────────────────────────────────────`
...
async function askClaude(userId: string, prompt: string, opts: ClaudeOptions) {
  const previousSession = sessions.get(userId);
  ...
  for await (const msg of conversation) {
    if (msg.session_id) {
      sessions.set(userId, msg.session_id);  // 更新 session
    }
    // ... 收集回复
  }
}
